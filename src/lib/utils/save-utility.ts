...
export async function quickSave(): Promise<GameSaveData & { id: number }> {
    ...
    const slots = await Promise.all(ids.map((id) => get(id)));
...
// save storage backend below, since it's a same-page fast path read on every route load.
export namespace autoExit {
    ...
    /** The auto-exit save, if any, without consuming it — used by {@link getLast}. */
...
export function create(options?: { image?: string; name?: string }): GameSaveData {
...
export async function save(
    ...
    return (await getLast()) as GameSaveData & { id: number };
}
